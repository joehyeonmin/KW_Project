...
// add
vec3	m_translucent(0.5, 0.5, 0.5);
...
vec3 intensity(const Ray& ray, const Light l[], int nLights, int depth, int E = -1)
{
	...
	if (iObject != -1)	// Hit an object
	{
		...
		if (depth < DEPTH)
		{
			// Ray tracing of reflective (but not translucent) spheres
			if (selection == 1)	// With a ray tracing depth 4
			{	
			...
			// Ray tracing of translucent (but not reflective) spheres
			else if (selection == 2)	// Without refraction with k# = 0.5
			{	
				Ray		tempRay(ray.p0 , ray.p1);

				vec3	I_T = intensity(tempRay, l, nLights, depth + 1, iObject);

				for (int i = 0; i < 3; i++)
					I[i] += m_translucent[i] * I_T[i];
			}

			// Ray tracing of translucent (but not reflective) spheres
			else if (selection == 3)	// With a relative refractive index input = 2 / reflect = 3
			{
				vec3	v = normalize(ray.p0 - ray.p1);
				float	temp_index = 2.0 / 3.0;
				float	cos_theta_i = dot(v, n);
			
				float	cos_theta_r = sqrt(1 - pow(temp_index, 2) * (1 - pow(cos_theta_i, 2)));
				vec3	temp = (temp_index * cos_theta_i - cos_theta_r) * n - temp_index * (ray.p0 - ray.p1);
				
				vec3	temp1 = normalize(temp);

				// make ray
				Ray temp1Ray(p + temp1 * 1.0e-4f, p + temp1 * 1.0e10f);
				
				vec3	I_T = intensity(temp1Ray, l, nLights, depth + 1, iObject);
			...
			// Ray tracing of reflectiveand refractive spheres
			else if (selection == 4)	// No reflections inside spheres
			{
				// reflective
				vec3	r = normalize(reflect(ray.p0 - ray.p1, n));
				vec3	pr = p + 1.0E10f * r;	// Point far away from p along r
...
#pragma omp master
	...
	// add
	cout << endl;
	cout << "Keyboard input: 1 for Ray tracing of reflective (but not translucent) spheres With a ray tracing depth 4" << endl;
	cout << "Keyboard input: 2 for Ray tracing of translucent (but not reflective) spheres Without refraction with k# = 0.5" << endl;
	cout << "Keyboard input: 3 for Ray tracing of translucent (but not reflective) spheres With a relative refractive index 2/3" << endl;
	cout << "Keyboard input: 4 for Ray tracing of reflective and refractive spheres(No reflections inside spheres)" << endl;
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS || action == GLFW_REPEAT) {
		...
		case GLFW_KEY_T:
			...
			// OpenMP
			// (5) Compare the rendering time with and without OpenMP 
