...
int findIntersection(const Ray& ray, vec3& p, vec3& n, int E)
{
	...
	for (int i = 0; i < nSpheres; i++)
	{
		...
		if (E == 7) {
			...
			vec3 small_point(0.001, 0.001, 0.001);

			// not outside sphere point
			Ray tempRay(ray.p0 + small_point, ray.p1);

			t = findIntersection(tempRay, center, radius[i], p_i, n_i);
		}
...
//추가
vec3 color_phong(const vec3& n, const vec3& v, const Light& l, const vec3 r, int E)
{
	vec3	I = ambient(l);
	...
	if (lambertian > 0)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			I[i] += color_diffuse[E][i] * lambertian * l.diffuse[i];
			I[i] += m_specular[i] * specular * l.specular[i];
		}
	}
...
vec3 intensity(const Ray& ray, const Light l[], int nLights, int depth, int E = -1)
{
	...
	for (int i = 0; i < nLights; i++)
	{
		...
		if (jObject == 7)				// Not shadowed
		{
			...
			I += color_phong(n, v, l[i], r, iObject);
		}
...
#pragma omp master
	...
	//추가
	center_world[nSpheres] = vec3(0, 0, 0); radius[nSpheres] = 8.0f; nSpheres++;
