...
void init()
{
	...
	cout << "Keyboard Input: 3 for for disk" << endl;
	cout << "Keyboard Input: 4 for for bunny" << endl;
...
void drawDisk(float inner_radius, float outer_height, const vec3& color)
{
	//Material
	setDiffuseColor(color);

	//Disk using GLU quadrics
	gluDisk(disk, inner_radius, outer_height, 72, 5);
}
...
bool readMesh(const char* filename) 
{
	...
	/*for (int i = 0;i < nVertices;i++) {
		numList[i].push_back(i);
	}*/

	// 추가
	numList = new list<int>[nVertices];
	...
	// 추가
	for (int i = 0; i < nFaces; i++) {
		int a = face[0][i], b = face[1][i], c = face[2][i];
		...
		if (b < c)
			numList[b].push_back(c);
		
		if (c < b)
			numList[c].push_back(b);
		
		if (c < a)
			numList[c].push_back(a);
		
		if (a < c)
			numList[a].push_back(c);
	}

	for (int i = 0; i < nVertices; i++) {
		edge_count = edge_count + numList[i].size();
	}

	cout << "Edges = " << nEdges << " || My_Count  = " << edge_count / 2 << endl;
...
void drawMesh() {
	...
	for (int i = 0; i < nVertices; i++) {
		...
		for (it = numList[i].begin(); it != numList[i].end(); it++) {
			glVertex3f(vertex[i].x,   vertex[i].y,   vertex[i].z);
			glVertex3f(vertex[*it].x, vertex[*it].y, vertex[*it].z);
		}
	}
