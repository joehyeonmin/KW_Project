...
void loadCheckerboardTexture()
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, 64, 64, 0, GL_RGB, GL_UNSIGNED_BYTE, checkerboard);
...
void render(GLFWwindow* window)
{
	...
	/*if (selection <= 4)	// Tranparent bunny rotating about the y-azis
	{
		glPushMatrix();
	...
	switch (selection)
	{
	case 1: drawTexturedCube();			break;
	case 2: drawSortedTexturedCube();	break;
	}
